...
public class MediumBuilding extends Building {
	...
	public MediumBuilding() {
		super(Constants.MEDIUM_BUILDING,
			  Constants.MEDIUM_B_MAX_ROW,
			  Constants.MEDIUM_B_MAX_COL);
	}
	...
	public boolean isEntryPossible(Position position) {
		...
		for (int i = 0; i < this.locations.length; i++) {
			...
			if (arePositionsEqual(locations[i], position)) {
				return true;
			}
		}
	...
	private boolean isFirstDiagonalDestroyed() {
		...
		for (Position position : this.explodedPositions) {
			...
			if (arePositionsEqual(firstCorner, position) ||
				arePositionsEqual(secondCorner, position)) {
				corrnersCount++;
			}
		}
